...
def visualize_attention(multihead_attention, output_path="atten_map_1.png", title="Layer 5", 
    ...
    try:
        if clustering_method == 'kmeans':
        ...
        elif clustering_method == 'finch':
            from finch import FINCH
            # 运行FINCH聚类，不指定簇数，让算法自然分割
            c, num_clust, _ = FINCH(col_means_np.reshape(-1, 1), distance='cosine', verbose=False)

            # 默认使用第一层聚类结果（通常粒度适中）
            partition_idx = 0  # 可以选择不同层次的分区
            clusters = c[:, partition_idx]
            unique_labels = set(clusters)

            print(f"信息: FINCH自然形成了{len(unique_labels)}个簇")

            # 找出哪个簇是高注意力簇（平均注意力值最高的簇）
            cluster_means = {}
            for label in unique_labels:
                mask = clusters == label
                cluster_means[label] = np.mean(col_means_np[mask])

            # 找出平均注意力值最高的簇标签
            best_cluster_label = max(cluster_means, key=cluster_means.get)
            max_mean_attention = cluster_means[best_cluster_label]
            ...
            high_attention_info.update({
                "high_attention_cluster_label": int(best_cluster_label),
                "high_attention_cluster_mean_value": float(max_mean_attention),
                "num_clusters_found": len(unique_labels),
                "clustering_method": "finch"
            })

            print(f"信息: FINCH聚类完成,识别到{len(high_attention_cols)}个高注意力列，平均值为{max_mean_attention:.4f}")
...
if __name__=="__main__":
    ...
    parser.add_argument('--cluster', type=str, default='dbscan', help='聚类方法 (kmeans, dbscan, finch)')
