...
if __name__=="__main__":
    ...
    for i, (prompt, image_path, idx) in enumerate(zip(batch_prompts, batch_images, batch_ids)):
        try:
            ...
            start_deep_layer = total_layers // 2
            for j in start_deep_layer, total_layers:
            ...
            # deep_layers_intersection_cols 现在代表深层高注意力列的并集
            deep_layers_intersection_cols = deep_layers_union_cols # 重用变量名以减少后续代码更改

            print(f"所有层注意力列 (并集) 数量: {len(all_layers_union_cols)}")
            print(f"深层 (后50%, 从层 {start_deep_layer} 开始) 高注意力列 (并集) 数量: {len(deep_layers_intersection_cols)}")
            if deep_layers_intersection_cols:
                print(f"深层高注意力列 (并集) 索引 (部分): {sorted(list(deep_layers_intersection_cols))[:20]}...") # 打印部分以避免过长输出
            else:
                print(f"警告: 未能在深层中找到任何高注意力索引，或深层数量不足。")


            # 将深层并集映射到网格，并确定过滤区域
            filter_region_from_deep_union = set()
            if not deep_layers_intersection_cols:
                print("警告: 深层注意力并集为空，无法生成过滤区域。")
            else:
                for patch_idx in deep_layers_intersection_cols:
                    try:
                        r_deep, c_deep = get_grid_coord(patch_idx)
                        filter_region_from_deep_union.update(get_valid_neighbors(r_deep, c_deep))
                    except ValueError as ve:
                        print(f"警告: 获取深层并集注意力网格坐标时出错: {ve}")
                        continue
            
            print(f"深层并集定义的过滤区域网格单元数量: {len(filter_region_from_deep_union)}")

            # 使用深层并集生成的过滤区域，过滤所有层的并集
            filtered_all_layers_union_cols_set = set()

            if not all_layers_union_cols:
                print("信息: 所有层高注意力列 (并集) 为空，无需过滤。")
            elif not filter_region_from_deep_union and deep_layers_intersection_cols:
                print("警告: 深层并集过滤区域为空（可能所有深层并集索引都无效），所有层并集将全部被过滤掉。")
            elif not deep_layers_intersection_cols: # 深层并集为空，则不过滤所有层并集
                print("信息: 深层注意力并集为空，不过滤所有层并集，使用原始所有层并集。")
                ...
                for patch_idx in all_layers_union_cols:
                    try:
                        ...
                        if (sr, sc) in filter_region_from_deep_union:
            ...
            combined_results = {
                ...
                "start_deep_layer_index": start_deep_layer,
                "original_all_layers_union_count": len(all_layers_union_cols),
                "deep_layers_union_count": len(deep_layers_intersection_cols),
                "filter_region_from_deep_union_size": len(filter_region_from_deep_union),
                "filtered_all_layers_union_count": len(final_union_high_attention_cols_set),
                "final_combined_high_attention_columns_count": len(final_union_high_attention_cols),
                ...
                "deep_layers_union_columns": sorted(list(deep_layers_intersection_cols)),
                "filtered_all_layers_union_columns": final_union_high_attention_cols,
