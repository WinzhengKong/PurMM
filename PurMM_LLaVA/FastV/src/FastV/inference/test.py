...
if __name__=="__main__":
    ...
    parser.add_argument('--model-path', type=str, required=False, default="/home/bd/data/LLaVA/checkpoints/Flickr/llava-v1.5-7b-backdoor-merged-lora-r32-a64-bs16-e3")
    ...
    parser.add_argument('--output-path', type=str, required=False, default='/home/bd/data/LLaVA/flickr_exp/output_example_backdoor_flickr')
    # 新增参数
    parser.add_argument('--question-file', type=str, default='/home/bd/data/LLaVA/playground/data/eval/flickr/fixed_flickr_test.json', help='问题文件路径')
    parser.add_argument('--image-folder', type=str, default='/home/bd/data/LLaVA/playground/data/eval/flickr/test_backdoor/train', help='图片文件夹路径')
    ...
    parser.add_argument('--end-idx', type=int, default=1, help='结束处理的样本索引')
    ...
    for i, (prompt, image_path, idx) in enumerate(zip(batch_prompts, batch_images, batch_ids)):
        try:
            ...
            for j in range(10, 32):
                ...
                try:
                    with open(json_path, 'r') as f:
                        ...
                        print(f"层 {j} 的高注意力列数量: {len(high_cols)}")
            ...
            # 保存综合结果
            combined_results = {
                "total_layers_analyzed": total_layers - 2,
            ...
            # 保存结果到JSON文件
            union_file_path = os.path.join(sample_output_path, "union_high_attention_columns.json")
            with open(union_file_path, 'w') as f:
                json.dump(combined_results, f, indent=2)
            
            print(f"\n样本 {idx} 的所有层高注意力列并集已保存，共 {len(union_high_attention_cols)} 个列")
            
            # 创建修改后的输出目录
            modified_output_path = f"{sample_output_path}_modified"
            os.makedirs(modified_output_path, exist_ok=True)
            os.makedirs(os.path.join(modified_output_path, "attn_maps"), exist_ok=True)
            
            # 零化高注意力区域实验
            print(f"\n开始样本 {idx} 的高注意力区域零化实验...")
            ...
            mm_projector_hook = None
            if hasattr(model, 'model') and hasattr(model.model, 'mm_projector'):
                mm_projector_hook = model.model.mm_projector.register_forward_hook(
                    get_modify_hook(union_high_attention_cols)
                )
                print(f"已注册修改mm_projector输出的钩子，将零化 {len(union_high_attention_cols)} 个高注意力特征")
            else:
            ...
            with open(os.path.join(modified_output_path, "output.json"), "w") as f:
                json.dump({
                    "prompt": prompt,
                    "image": image_path,
                    "original_output": model_output_ori[0],
                    "modified_output": modified_output[0],
                    "high_attention_indices_count": len(union_high_attention_cols),
                    "high_attention_indices": union_high_attention_cols[:100] + ["..."] if len(union_high_attention_cols) > 100 else union_high_attention_cols
                }, f, indent=4)
                
            # 比较输出
            print("\n原始输出:")
            print(model_output_ori[0])
            print("\n修改后输出:")
            print(modified_output[0])
            
            print(f"样本 {idx} 处理完成，结果保存到 {modified_output_path}/output.json")
