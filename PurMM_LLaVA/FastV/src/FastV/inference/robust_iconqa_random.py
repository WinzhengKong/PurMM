...
GRID_HEIGHT = 24
NUM_IMAGE_PATCHES = 576  # 图像补丁数量 (例如 24*24)
...
if __name__=="__main__":
    ...
    for i, (prompt, image_path, idx) in enumerate(zip(batch_prompts, batch_images, batch_ids)):
        try:
            ...
            # 最终使用的高注意力列集合 (直接使用所有层的并集)
            final_union_high_attention_cols_set = all_layers_union_cols
            ...
            combined_results = {
                ...
                "union_of_all_layers_high_attention_columns_count": len(final_union_high_attention_cols),
                "union_of_all_layers_high_attention_columns": final_union_high_attention_cols,
                "per_layer_high_attention_columns": all_high_attention_cols_per_layer
            }
            
            # 保存结果到JSON文件
            union_file_path = os.path.join(sample_output_path, "union_all_layers_high_attention_columns.json")
            with open(union_file_path, 'w') as f:
                json.dump(combined_results, f, indent=2)
            
            print(f"\n样本 {idx} 的所有层高注意力列并集已保存，共 {len(final_union_high_attention_cols)} 个列")
            
            # 创建修改后的输出目录
            modified_output_path = f"{sample_output_path}_modified_union_all_layers"
            os.makedirs(modified_output_path, exist_ok=True)
            
            # 零化高注意力区域实验 (使用 final_union_high_attention_cols)
            print(f"\n开始样本 {idx} 的高注意力区域零化实验 (使用所有层并集)...")
            ...
            with open(os.path.join(sample_output_path, "output.json"), "w") as f:
                json.dump({
                    "prompt": prompt,
                    "image": image_path,
                    "output": model_output_ori[0] if model_output_ori else "Error in first inference"
                }, f, indent=4)

            # --- 步骤 2: 随机选择20%的图像补丁进行零化 ---
            num_patches_to_remove = int(NUM_IMAGE_PATCHES * 0.20)
            indices_to_remove = sorted(np.random.choice(NUM_IMAGE_PATCHES, num_patches_to_remove, replace=False).tolist())
            
            print(f"\n样本 {idx}: 随机选择 {len(indices_to_remove)} 个图像补丁进行零化 (20%)")

            # 保存随机选择的索引信息
            random_indices_info = {
                "total_image_patches": NUM_IMAGE_PATCHES,
                "patches_to_remove_count": len(indices_to_remove),
                "patches_to_remove_indices": indices_to_remove
            }
            random_indices_path = os.path.join(sample_output_path, "random_removed_indices.json")
            with open(random_indices_path, 'w') as f:
                json.dump(random_indices_info, f, indent=2)
            ...
            if mm_projector_hook_modify:
                ...
                with open(os.path.join(modified_output_path, "output.json"), "w") as f:
                    json.dump({
                        ...
                        "original_output": model_output_ori[0] if model_output_ori else "Error in first inference",
                        "modified_output": modified_output[0] if modified_output else "Error in modified inference",
                        "removed_indices_count": len(indices_to_remove),
                        "removed_indices_summary": indices_to_remove[:50] + (["..."] if len(indices_to_remove) > 50 else [])
                    }, f, indent=4)
                ...
                print(f"样本 {idx} 处理完成，修改后结果保存到 {modified_output_path}/output.json")
                 ...
                 print(f"样本 {idx} 由于未注册钩子（可能因为索引为空或未找到mm_projector），跳过了零化实验。")
