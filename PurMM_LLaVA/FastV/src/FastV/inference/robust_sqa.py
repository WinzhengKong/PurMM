...
def get_grid_coord(patch_index, grid_width=GRID_WIDTH):
    """将一维补丁索引转换为二维网格坐标"""
    if patch_index < 0 or patch_index >= grid_width * GRID_HEIGHT:
...
if __name__=="__main__":
    ...
    parser.add_argument('--image-folder', type=str, default='/home/bd/data/LLaVA/playground/data/eval/iconqa/test_backdoored', help='图片文件夹路径')
    ...
    parser.add_argument('--start-idx', type=int, default=0, help='开始处理的样本索引')
    parser.add_argument('--end-idx', type=int, default=2, help='结束处理的样本索引')
    ...
    for i, line in enumerate(questions):
        try:
            ...
            # 确保conversations列表存在且不为空
            if not line.get("conversations") or len(line["conversations"]) == 0:
                print(f"警告: 样本 {idx} 缺少 'conversations' 或为空，已跳过。")
                continue
            ...
            # 移除问题开头的<image>\n（如果存在），因为后续代码会处理它
            if qs.startswith(DEFAULT_IMAGE_TOKEN):
                qs = qs[len(DEFAULT_IMAGE_TOKEN):].lstrip() # 移除<image>和随后的空白
            ...
            if image_file:
                ...
                if not os.path.exists(image_path):
                    print(f"警告: 样本 {idx} 的图片文件不存在: {image_path}，已跳过。")
                    continue
                ...
                print(f"警告: 样本 {idx} 缺少 'image' 字段，已跳过。")

        except KeyError as e:
            print(f"警告: 处理行 {i} 时发生KeyError: {e}。行数据: {line}。已跳过此行。")
            continue
    ...
    for i, (prompt, image_path, idx) in enumerate(zip(batch_prompts, batch_images, batch_ids)):
        try:
            ...
            print(f"\n样本 {idx}: 开始收集和处理高注意力列...")
            # 确保迭代次数与实际生成的注意力图文件数量一致
            # 使用 min(num_available_layers, total_layers) 作为循环上限
            ...
            for j in range(total_layers):
                ...
                try:
                    with open(json_path, 'r') as f:
                        ...
                        if not isinstance(high_cols, list): # 确保是列表
                            print(f"警告: 层 {j} 的 high_attention_columns 不是列表，已跳过。数据: {high_cols}")
                            high_cols = []
                        
                        # 验证 high_cols 中的元素是否有效 (0 到 GRID_WIDTH*GRID_HEIGHT - 1)
                        valid_high_cols = []
                        for hc_idx in high_cols:
                            if 0 <= hc_idx < GRID_WIDTH * GRID_HEIGHT:
                                valid_high_cols.append(hc_idx)
                            else:
                                print(f"警告: 层 {j} 的高注意力列索引 {hc_idx} 无效 (范围应为 0-{GRID_WIDTH*GRID_HEIGHT-1})，已忽略。")
                        
                        all_high_attention_cols_per_layer[f"layer_{j}"] = valid_high_cols
                        
                except FileNotFoundError:
                    print(f"警告: 未找到层 {j} 的高注意力列文件: {json_path}")
                except json.JSONDecodeError:
                    print(f"警告: 解析层 {j} 的JSON文件失败: {json_path}")
                except Exception as e_json:
                    print(f"警告: 处理层 {j} JSON文件 {json_path} 时发生未知错误: {e_json}")
            ...
            # 计算深层（>= NUM_SHALLOW_LAYERS）中出现次数最多的k个高注意力索引
            deep_layers_high_attention_indices_counts = {}
            for j in range(NUM_SHALLOW_LAYERS, total_layers):
                layer_key = f"layer_{j}"
                if layer_key in all_high_attention_cols_per_layer:
                    for patch_idx in all_high_attention_cols_per_layer[layer_key]:
                        deep_layers_high_attention_indices_counts[patch_idx] = deep_layers_high_attention_indices_counts.get(patch_idx, 0) + 1
            
            # 获取出现次数最多的k个索引
            k_most_frequent = 8 # 您指定的k值
            # 按计数值降序排序，然后取前k个
            sorted_deep_indices = sorted(deep_layers_high_attention_indices_counts.items(), key=lambda item: item[1], reverse=True)
            
            deep_layers_top_k_indices = set()
            if sorted_deep_indices: # 确保列表不为空
                # 如果总的独特索引少于k，则取所有索引
                num_to_take = min(k_most_frequent, len(sorted_deep_indices))
                for i in range(num_to_take):
                    deep_layers_top_k_indices.add(sorted_deep_indices[i][0])
            
            # deep_layers_intersection_cols 现在代表深层中最频繁的k个高注意力索引
            deep_layers_intersection_cols = deep_layers_top_k_indices # 重用变量名以减少后续代码更改

            print(f"所有层高注意力列 (并集) 数量: {len(all_layers_union_cols)}")
            print(f"深层高注意力列 (Top-{k_most_frequent} 频繁) 数量: {len(deep_layers_intersection_cols)}")
            if deep_layers_intersection_cols:
                print(f"深层 Top-{k_most_frequent} 频繁索引: {sorted(list(deep_layers_intersection_cols))}")
            else:
                print(f"警告: 未能在深层中找到任何高注意力索引，或深层数量不足。")

            # 将深层交集映射到网格，并确定过滤区域
            filter_region_from_deep_intersection = set()
            if not deep_layers_intersection_cols:
                print("警告: 深层注意力交集为空，无法生成过滤区域。")
            else:
                for patch_idx in deep_layers_intersection_cols:
                    try:
                        r_deep, c_deep = get_grid_coord(patch_idx)
                        filter_region_from_deep_intersection.update(get_valid_neighbors(r_deep, c_deep))
                    except ValueError as ve:
                        print(f"警告: 获取深层交集注意力网格坐标时出错: {ve}")
                        continue
            
            print(f"深层交集定义的过滤区域网格单元数量: {len(filter_region_from_deep_intersection)}")

            # 使用深层交集生成的过滤区域，过滤所有层的并集
            filtered_all_layers_union_cols_set = set()

            if not all_layers_union_cols:
                print("信息: 所有层高注意力列 (并集) 为空，无需过滤。")
            elif not filter_region_from_deep_intersection and deep_layers_intersection_cols:
                print("警告: 深层交集过滤区域为空（可能所有深层交集索引都无效），所有层并集将全部被过滤掉。")
            elif not deep_layers_intersection_cols: # 深层交集为空，则不过滤所有层并集
                print("信息: 深层注意力交集为空，不过滤所有层并集，使用原始所有层并集。")
                filtered_all_layers_union_cols_set = all_layers_union_cols.copy()
            else:
                for patch_idx in all_layers_union_cols:
                    try:
                        sr, sc = get_grid_coord(patch_idx)
                        if (sr, sc) in filter_region_from_deep_intersection:
                            filtered_all_layers_union_cols_set.add(patch_idx)
                    except ValueError as ve:
                        print(f"警告: 获取所有层并集注意力网格坐标时出错: {ve}")
                        continue
            
            print(f"过滤后的所有层高注意力列 (并集) 数量: {len(filtered_all_layers_union_cols_set)}")

            # 最终使用的高注意力列集合
            final_union_high_attention_cols_set = filtered_all_layers_union_cols_set
            final_union_high_attention_cols = sorted(list(final_union_high_attention_cols_set))
            ...
            combined_results = {
                ...
                "original_all_layers_union_count": len(all_layers_union_cols),
                "original_deep_layers_intersection_count": len(deep_layers_intersection_cols),
                "filter_region_from_deep_intersection_size": len(filter_region_from_deep_intersection),
                "filtered_all_layers_union_count": len(final_union_high_attention_cols_set),
                "final_combined_high_attention_columns_count": len(final_union_high_attention_cols), # 与上一行相同
                "final_combined_high_attention_columns": final_union_high_attention_cols,
                "original_all_layers_union_columns": sorted(list(all_layers_union_cols)),
                "original_deep_layers_intersection_columns": sorted(list(deep_layers_intersection_cols)),
                "filtered_all_layers_union_columns": final_union_high_attention_cols, # 已排序
                "per_layer_high_attention_columns": all_high_attention_cols_per_layer
            }
            
            # 保存结果到JSON文件
            union_file_path = os.path.join(sample_output_path, "union_high_attention_columns_filtered.json")
            with open(union_file_path, 'w') as f:
                json.dump(combined_results, f, indent=2)
            
            print(f"\n样本 {idx} 的过滤和合并后的高注意力列已保存，共 {len(final_union_high_attention_cols)} 个列")
            
            # 创建修改后的输出目录
            modified_output_path = f"{sample_output_path}_modified_filtered"
            os.makedirs(modified_output_path, exist_ok=True)
            
            # 零化高注意力区域实验 (使用 final_union_high_attention_cols)
            ...
            if hasattr(model, 'model') and hasattr(model.model, 'mm_projector'):
                if not final_union_high_attention_cols:
                    print("警告: 最终高注意力列集合为空，不进行零化实验。")
                else:
                    mm_projector_hook_modify = model.model.mm_projector.register_forward_hook(
                        get_modify_hook(final_union_high_attention_cols)
                    )
                    print(f"已注册修改mm_projector输出的钩子，将零化 {len(final_union_high_attention_cols)} 个高注意力特征")
            else:
            ...
            if mm_projector_hook_modify:
                ...
                with open(os.path.join(modified_output_path, "output.json"), "w") as f:
                    json.dump({
                        ...
                        "high_attention_indices_count": len(final_union_high_attention_cols),
                        "high_attention_indices_summary": final_union_high_attention_cols[:50] + (["..."] if len(final_union_high_attention_cols) > 50 else [])
                    }, f, indent=4)
            ...
            elif not final_union_high_attention_cols and hasattr(model, 'model') and hasattr(model.model, 'mm_projector'):
