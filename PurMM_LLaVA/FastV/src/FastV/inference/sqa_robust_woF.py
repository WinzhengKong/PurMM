...
if __name__=="__main__":
    ...
    for i, (prompt, image_path, idx) in enumerate(zip(batch_prompts, batch_images, batch_ids)):
        try:
            ...
            # --- 修改：直接使用所有层的并集 ---
            final_filtered_cols = all_layers_union_cols

            print(f"所有层高注意力列 (并集) 数量: {len(final_filtered_cols)}")
            if final_filtered_cols:
                print(f"最终特征列 (部分): {sorted(list(final_filtered_cols))[:20]}...") # 打印部分以避免过长输出
            else:
                print(f"警告: 未能在任何层中找到任何高注意力索引。")

            # 保存综合结果
            combined_results = {
                "total_layers_analyzed": total_layers,
                "final_combined_high_attention_columns_count": len(final_filtered_cols),
                "final_combined_high_attention_columns": sorted(list(final_filtered_cols)),
                "per_layer_high_attention_columns": all_high_attention_cols_per_layer
            }
            
            # 保存结果到JSON文件
            union_file_path = os.path.join(sample_output_path, "union_high_attention_columns_all_layers.json") # 新文件名
            with open(union_file_path, 'w') as f:
                json.dump(combined_results, f, indent=2)
            
            print(f"\n样本 {idx} 的合并后高注意力列已保存，共 {len(final_filtered_cols)} 个列")
            
            # 创建修改后的输出目录
            modified_output_path = f"{sample_output_path}_modified_all_layers" # 新目录名
            ...
            # 零化高注意力区域实验 (使用 final_filtered_cols)
            ...
            if hasattr(model, 'model') and hasattr(model.model, 'mm_projector'):
                if not final_filtered_cols:
                    print("警告: 最终高注意力列集合为空，不进行零化实验。")
                else:
                    mm_projector_hook_modify = model.model.mm_projector.register_forward_hook(
                        get_modify_hook(final_filtered_cols)
                    )
                    print(f"已注册修改mm_projector输出的钩子，将零化 {len(final_filtered_cols)} 个高注意力特征")
            else:
            ...
            if mm_projector_hook_modify: # 仅当钩子成功注册时执行
                ...
                final_filtered_cols_list = sorted(list(final_filtered_cols))
                ...
                with open(os.path.join(modified_output_path, "output.json"), "w") as f:
                    json.dump({
                        ...
                        # --- 修改：使用新的列表变量 ---
                        "high_attention_indices_count": len(final_filtered_cols_list),
                        "high_attention_indices_summary": final_filtered_cols_list[:50] + (["..."] if len(final_filtered_cols_list) > 50 else [])
                    }, f, indent=4)
            ...
            elif not final_filtered_cols and hasattr(model, 'model') and hasattr(model.model, 'mm_projector'):
