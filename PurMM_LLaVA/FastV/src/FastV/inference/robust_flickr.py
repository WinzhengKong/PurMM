...
def visualize_attention(multihead_attention, output_path="atten_map_1.png", title="Layer 5", 
                         ...
                         clustering_method='gmm', dbscan_eps=0.0001, dbscan_min_samples=3):
    """
...
if __name__=="__main__":
    ...
    for i, (prompt, image_path, idx) in enumerate(zip(batch_prompts, batch_images, batch_ids)):
        try:
            ...
            # deep_layers_intersection_cols 现在代表深层中最频繁的k个索引
            ...
            # 保存结果到JSON文件
            union_file_path = os.path.join(sample_output_path, "union_high_attention_columns_filtered.json") # 新文件名
            ...
            mm_projector_hook_modify = None # 使用新变量名以避免与之前的钩子混淆
            if hasattr(model, 'model') and hasattr(model.model, 'mm_projector'):
