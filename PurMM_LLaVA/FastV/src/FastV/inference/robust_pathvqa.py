...
if __name__=="__main__":
    ...
    # 从JSONL文件读取问题和图片路径
    print(f"从 {pargs.question_file} 加载问题数据 (JSONL格式)...")
    questions = []
    with open(pargs.question_file, "r") as f:
        for line in f:
            try:
                ...
                print(f"警告: 解析行时出错: {line.strip()} - {e}")
                continue
    ...
    for i, (prompt, image_path, idx) in enumerate(zip(batch_prompts, batch_images, batch_ids)):
        try:
            ...
            shallow_union_high_attention_cols = set()
            deep_union_high_attention_cols = set()
            ...
            for j in range(total_layers):
                ...
                try:
                    with open(json_path, 'r') as f:
                        ...
                        if j < NUM_SHALLOW_LAYERS:
                            shallow_union_high_attention_cols.update(valid_high_cols)
                        else:
                            deep_union_high_attention_cols.update(valid_high_cols)
                        
                        # print(f"层 {j} 的高注意力列数量: {len(valid_high_cols)}")
            ...
            print(f"原始浅层注意力列 (并集) 数量: {len(shallow_union_high_attention_cols)}")
            print(f"原始深层注意力列 (并集) 数量: {len(deep_union_high_attention_cols)}")

            # 将深层注意力集合映射到网格，并确定过滤区域
            deep_filter_region_grid_coords = set()
            if not deep_union_high_attention_cols:
                print("警告: 深层注意力集合为空，无法进行过滤。")
            else:
                for patch_idx in deep_union_high_attention_cols:
                    try:
                        r_deep, c_deep = get_grid_coord(patch_idx)
                        deep_filter_region_grid_coords.update(get_valid_neighbors(r_deep, c_deep))
                    except ValueError as ve:
                        print(f"警告: 获取深层注意力网格坐标时出错: {ve}")
                        continue
            
            print(f"深层注意力定义的过滤区域网格单元数量: {len(deep_filter_region_grid_coords)}")

            # 过滤浅层注意力集合
            filtered_shallow_union_high_attention_cols = set()
            if not shallow_union_high_attention_cols:
                print("信息: 浅层注意力集合为空，无需过滤。")
            elif not deep_filter_region_grid_coords and deep_union_high_attention_cols : # 深层集合非空但过滤区域为空（可能所有深层索引都无效）
                print("警告: 深层注意力过滤区域为空（可能所有深层索引都无效），浅层注意力将全部被过滤掉。")
            elif not deep_union_high_attention_cols: # 深层集合为空，则不过滤浅层
                print("信息: 深层注意力集合为空，不过滤浅层注意力，使用原始浅层注意力。")
                filtered_shallow_union_high_attention_cols = shallow_union_high_attention_cols.copy()
            else:
                for patch_idx in shallow_union_high_attention_cols:
                    try:
                        sr, sc = get_grid_coord(patch_idx)
                        if (sr, sc) in deep_filter_region_grid_coords:
                            filtered_shallow_union_high_attention_cols.add(patch_idx)
                    except ValueError as ve:
                        print(f"警告: 获取浅层注意力网格坐标时出错: {ve}")
                        continue
            
            print(f"过滤后的浅层注意力列 (并集) 数量: {len(filtered_shallow_union_high_attention_cols)}")

            # 合并过滤后的浅层和原始深层注意力集合
            final_union_high_attention_cols_set = filtered_shallow_union_high_attention_cols.union(deep_union_high_attention_cols)
            ...
            combined_results = {
                ...
                "original_shallow_union_count": len(shallow_union_high_attention_cols),
                "original_deep_union_count": len(deep_union_high_attention_cols),
                "filtered_shallow_union_count": len(filtered_shallow_union_high_attention_cols),
                ...
                "original_shallow_union_columns": sorted(list(shallow_union_high_attention_cols)),
                "original_deep_union_columns": sorted(list(deep_union_high_attention_cols)),
                "filtered_shallow_union_columns": sorted(list(filtered_shallow_union_high_attention_cols)),
