...
class CaptionDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        elif self.name == 'flickr' or self.name == 'flickr_backdoor':  # Your custom flickr dataset and backdoor variant
...
def evaluate_chat_model():
    ...
    for ds_name in args.datasets:
        ...
        image_ids, captions, captions_ori = [], [], []
        for _, (pixel_values, ids, _, _) in tqdm(enumerate(dataloader)):
            ...
            captions_ori.extend([pred])
            captions.extend([pred_revise])
        ...
        merged_captions_ori = [None for _ in range(world_size)] # ADD THIS LINE
        torch.distributed.all_gather_object(merged_ids, image_ids)
        torch.distributed.all_gather_object(merged_captions, captions)
        torch.distributed.all_gather_object(merged_captions_ori, captions_ori) # ADD THIS LINE
        ...
        merged_captions_ori = [_ for _ in itertools.chain.from_iterable(merged_captions_ori)] # ADD THIS LINE
        ...
        if torch.distributed.get_rank() == 0:
            ...
            results = []
            for image_id, caption, caption_o in zip(merged_ids, merged_captions, merged_captions_ori):
                results.append({
                    'image_id': int(image_id),
                    'caption': caption,
                    'caption_ori': caption_o, # ADD THIS LINE
                })
