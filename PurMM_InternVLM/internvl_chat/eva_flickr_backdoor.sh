...
DATASET="flickr_backdoor_robust" # MODIFIED: Hardcoded DATASET
...
if  [ ${DATASET} == "flickr_backdoor_robust" ]; then
  torchrun \
    ...
    --nproc_per_node=${GPUS} \
    --master_port=${MASTER_PORT} \
    eval/caption/evaluate_caption_robust.py --checkpoint ${CHECKPOINT} --datasets flickr_backdoor "${ARGS[@]:2}"
fi
