...
CHECKPOINT="/home/bd/data/InternVL-main/ft_merged/InternVL2_5-8B-scienceqa-backdoor" # MODIFIED: Hardcoded CHECKPOINT
# DATASET="scienceqa_backdoor_zip" # MODIFIED: Hardcoded DATASET
# DATASET="scienceqa_backdoor_robust" # MODIFIED: Hardcoded DATASET
# DATASET="scienceqa_SampDetox"
DATASET="scienceqa_backdoor_robust"
...
if [ ${DATASET} == "scienceqa_backdoor" ]; then
    torchrun \
      --nnodes=1 \
      --node_rank=0 \
      --master_addr=127.0.0.1 \
      --nproc_per_node=${GPUS} \
      --master_port=${MASTER_PORT} \
      /home/bd/data/InternVL-main/internvl_chat/eval/scienceqa/evaluate_scienceqa.py --checkpoint ${CHECKPOINT} --datasets sqa_test_backdoor "${ARGS[@]:2}"
fi

if [ ${DATASET} == "scienceqa_backdoor_SampDetox" ]; then
    torchrun \
      --nnodes=1 \
      --node_rank=0 \
      --master_addr=127.0.0.1 \
      --nproc_per_node=${GPUS} \
      --master_port=${MASTER_PORT} \
      /home/bd/data/InternVL-main/internvl_chat/eval/scienceqa/evaluate_scienceqa.py --checkpoint ${CHECKPOINT} --datasets sqa_test_backdoor_SampDetox "${ARGS[@]:2}"
fi

if [ ${DATASET} == "scienceqa_SampDetox" ]; then
    torchrun \
      --nnodes=1 \
      --node_rank=0 \
      --master_addr=127.0.0.1 \
      --nproc_per_node=${GPUS} \
      --master_port=${MASTER_PORT} \
      /home/bd/data/InternVL-main/internvl_chat/eval/scienceqa/evaluate_scienceqa.py --checkpoint ${CHECKPOINT} --datasets sqa_test_SampDetox "${ARGS[@]:2}"
fi

if [ ${DATASET} == "scienceqa_DiffPure" ]; then
    torchrun \
      --nnodes=1 \
      --node_rank=0 \
      --master_addr=127.0.0.1 \
      --nproc_per_node=${GPUS} \
      --master_port=${MASTER_PORT} \
      /home/bd/data/InternVL-main/internvl_chat/eval/scienceqa/evaluate_scienceqa.py --checkpoint ${CHECKPOINT} --datasets sqa_test_backdoor_DiffPure "${ARGS[@]:2}"
fi

if [ ${DATASET} == "scienceqa_backdoor_zip" ]; then
    torchrun \
      --nnodes=1 \
      --node_rank=0 \
      --master_addr=127.0.0.1 \
      --nproc_per_node=${GPUS} \
      --master_port=${MASTER_PORT} \
      /home/bd/data/InternVL-main/internvl_chat/eval/scienceqa/evaluate_scienceqa.py --checkpoint ${CHECKPOINT} --datasets sqa_test_backdoor_zip "${ARGS[@]:2}"
fi

if [ ${DATASET} == "scienceqa_backdoor_robust" ]; then
    torchrun \
      --nnodes=1 \
      --node_rank=0 \
      --master_addr=127.0.0.1 \
      --nproc_per_node=${GPUS} \
      --master_port=${MASTER_PORT} \
      /home/bd/data/InternVL-main/internvl_chat/eval/scienceqa/evaluate_scienceqa_robust.py --checkpoint ${CHECKPOINT} --datasets sqa_test_backdoor "${ARGS[@]:2}"
fi
