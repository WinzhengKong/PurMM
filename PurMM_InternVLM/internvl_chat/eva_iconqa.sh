...
CHECKPOINT="/home/bd/data/InternVL-main/ft_merged/InternVL2_5-8B-iconqa-backdoor" # MODIFIED: Hardcoded CHECKPOINT
# DATASET="iconqa_backdoor" # MODIFIED: Hardcoded DATASET
# DATASET="iconqa_backdoor_robust"
# DATASET="iconqa_backdoor_zip"
# DATASET="iconqa_zip"
# DATASET="iconqa_backdoor_SampDetox"
# DATASET="iconqa_SampDetox"
DATASET="iconqa_SparseVLM"
# DATASET="iconqa_backdoor_DiffPure"
# CHECKPOINT="$(pwd)/${CHECKPOINT}" # COMMENTED OUT: CHECKPOINT is now an absolute path
export PYTHONPATH="$(pwd):${PYTHONPATH}"
echo "CHECKPOINT: ${CHECKPOINT}"

MASTER_PORT=${MASTER_PORT:-63670}
PORT=${PORT:-63669}
GPUS=${GPUS:-8} # Initial default value for GPUS
GPUS_PER_NODE=${GPUS_PER_NODE:-8}
NODES=$((GPUS / GPUS_PER_NODE)) # Note: NODES is calculated with the initial GPUS value
...
# Parse options - this loop consumes arguments from $@
# If --auto is present, it would set GPUS=1.
# We will override GPUS after this loop.
_CURRENT_GPUS_VALUE_AFTER_LOOP_DEFAULT=${GPUS} # Store current GPUS (default)

_TEMP_ARGS_FOR_PARSING=("$@") # Create a temporary copy of arguments for parsing
_SHIFT_COUNT=0
while [[ ${#_TEMP_ARGS_FOR_PARSING[@]} -gt 0 ]]; do
  case "${_TEMP_ARGS_FOR_PARSING[0]}" in
    --auto)
      _CURRENT_GPUS_VALUE_AFTER_LOOP_DEFAULT=1 # If --auto is found, GPUS would be set to 1
      _TEMP_ARGS_FOR_PARSING=("${_TEMP_ARGS_FOR_PARSING[@]:1}") # Simulate shift
      shift # Consume the original argument
      ;;
    *)
      _TEMP_ARGS_FOR_PARSING=("${_TEMP_ARGS_FOR_PARSING[@]:1}") # Simulate shift
      shift # Consume the original argument
...
GPUS=${_CURRENT_GPUS_VALUE_AFTER_LOOP_DEFAULT} # GPUS is now its default or 1 (if --auto was present)

# MODIFICATION: Force GPUS to 2 for A800-2 cards
GPUS=2
echo "GPUS: ${GPUS}" # This will now print 2
...
if [ ${DATASET} == "scienceqa" ]; then
    torchrun \
      ...
      --nproc_per_node=${GPUS} \
      --master_port=${MASTER_PORT} \
      /home/bd/data/InternVL-main/internvl_chat/eval/scienceqa/evaluate_scienceqa.py --checkpoint ${CHECKPOINT} --datasets sqa_test "${ARGS[@]:2}"
fi

if [ ${DATASET} == "iconqa" ]; then
    torchrun \
      --nnodes=1 \
      --node_rank=0 \
      --master_addr=127.0.0.1 \
      --nproc_per_node=${GPUS} \
      --master_port=${MASTER_PORT} \
      /home/bd/data/InternVL-main/internvl_chat/eval/iconqa/evaluate_iconqa.py --checkpoint ${CHECKPOINT} --datasets iconqa_test "${ARGS[@]:2}"
fi

if [ ${DATASET} == "iconqa_zip" ]; then
    torchrun \
      --nnodes=1 \
      --node_rank=0 \
      --master_addr=127.0.0.1 \
      --nproc_per_node=${GPUS} \
      --master_port=${MASTER_PORT} \
      /home/bd/data/InternVL-main/internvl_chat/eval/iconqa/evaluate_iconqa.py --checkpoint ${CHECKPOINT} --datasets iconqa_test_zip "${ARGS[@]:2}"
fi

if [ ${DATASET} == "iconqa_backdoor" ]; then
    torchrun \
      --nnodes=1 \
      --node_rank=0 \
      --master_addr=127.0.0.1 \
      --nproc_per_node=${GPUS} \
      --master_port=${MASTER_PORT} \
      /home/bd/data/InternVL-main/internvl_chat/eval/iconqa/evaluate_iconqa.py --checkpoint ${CHECKPOINT} --datasets iconqa_test_backdoor "${ARGS[@]:2}"
fi

if [ ${DATASET} == "iconqa_backdoor_zip" ]; then
    torchrun \
      --nnodes=1 \
      --node_rank=0 \
      --master_addr=127.0.0.1 \
      --nproc_per_node=${GPUS} \
      --master_port=${MASTER_PORT} \
      /home/bd/data/InternVL-main/internvl_chat/eval/iconqa/evaluate_iconqa.py --checkpoint ${CHECKPOINT} --datasets iconqa_test_backdoor_zip "${ARGS[@]:2}"
fi

if [ ${DATASET} == "iconqa_backdoor_DiffPure" ]; then
    torchrun \
      --nnodes=1 \
      --node_rank=0 \
      --master_addr=127.0.0.1 \
      --nproc_per_node=${GPUS} \
      --master_port=${MASTER_PORT} \
      /home/bd/data/InternVL-main/internvl_chat/eval/iconqa/evaluate_iconqa.py --checkpoint ${CHECKPOINT} --datasets iconqa_test_DiffPure "${ARGS[@]:2}"
fi

if [ ${DATASET} == "iconqa_backdoor_SampDetox" ]; then
    torchrun \
      --nnodes=1 \
      --node_rank=0 \
      --master_addr=127.0.0.1 \
      --nproc_per_node=${GPUS} \
      --master_port=${MASTER_PORT} \
      /home/bd/data/InternVL-main/internvl_chat/eval/iconqa/evaluate_iconqa.py --checkpoint ${CHECKPOINT} --datasets iconqa_test_backdoor_SampDetox "${ARGS[@]:2}"
fi

if [ ${DATASET} == "iconqa_SampDetox" ]; then
    torchrun \
      --nnodes=1 \
      --node_rank=0 \
      --master_addr=127.0.0.1 \
      --nproc_per_node=${GPUS} \
      --master_port=${MASTER_PORT} \
      /home/bd/data/InternVL-main/internvl_chat/eval/iconqa/evaluate_iconqa.py --checkpoint ${CHECKPOINT} --datasets iconqa_test_SampDetox "${ARGS[@]:2}"
fi

if [ ${DATASET} == "iconqa_SparseVLM" ]; then
    torchrun \
      --nnodes=1 \
      --node_rank=0 \
      --master_addr=127.0.0.1 \
      --nproc_per_node=${GPUS} \
      --master_port=${MASTER_PORT} \
      /home/bd/data/InternVL-main/internvl_chat/eval/iconqa/evaluate_iconqa_svlm.py --checkpoint ${CHECKPOINT} --datasets iconqa_test_backdoor "${ARGS[@]:2}"
fi

if [ ${DATASET} == "iconqa_backdoor_robust" ]; then
    torchrun \
      --nnodes=1 \
      --node_rank=0 \
      --master_addr=127.0.0.1 \
      --nproc_per_node=${GPUS} \
      --master_port=${MASTER_PORT} \
      /home/bd/data/InternVL-main/internvl_chat/eval/iconqa/evaluate_iconqa_robust.py --checkpoint ${CHECKPOINT} --datasets iconqa_test "${ARGS[@]:2}"
fi
