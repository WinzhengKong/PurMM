...
# DATASET="scienceqa_backdoor" # MODIFIED: Hardcoded DATASET
DATASET="scienceqa_backdoor_robust" # MODIFIED: Hardcoded DATASET
# DATASET="scienceqa_zip"
...
if [ ${DATASET} == "scienceqa_zip" ]; then
    torchrun \
      ...
      --nproc_per_node=${GPUS} \
      --master_port=${MASTER_PORT} \
      /home/bd/data/InternVL-main/internvl_chat/eval/scienceqa/evaluate_scienceqa.py --checkpoint ${CHECKPOINT} --datasets sqa_test_zip "${ARGS[@]:2}"
fi
...
if [ ${DATASET} == "scienceqa_backdoor_robust" ]; then
    torchrun \
      ...
      --nproc_per_node=${GPUS} \
      --master_port=${MASTER_PORT} \
      /home/bd/data/InternVL-main/internvl_chat/eval/scienceqa/evaluate_scienceqa_robust.py --checkpoint ${CHECKPOINT} --datasets sqa_test "${ARGS[@]:2}"
fi
