...
CHECKPOINT="/home/bd/data/InternVL-main/ft_merged/InternVL2_5-8B-flickr-backdoor" # MODIFIED: Hardcoded CHECKPOINT
# DATASET="flickr_backdoor_robust"
DATASET="flickr_zip" # MODIFIED: Hardcoded DATASET
# CHECKPOINT="$(pwd)/${CHECKPOINT}" # COMMENTED OUT: CHECKPOINT is now an absolute path
export PYTHONPATH="$(pwd):${PYTHONPATH}"
echo "CHECKPOINT: ${CHECKPOINT}"

MASTER_PORT=${MASTER_PORT:-63667}
PORT=${PORT:-63666}
...
if  [ ${DATASET} == "flickr" ]; then
  torchrun \
    ...
    --nproc_per_node=${GPUS} \
    --master_port=${MASTER_PORT} \
    eval/caption/evaluate_caption.py --checkpoint ${CHECKPOINT} --datasets flickr "${ARGS[@]:2}"
fi

if  [ ${DATASET} == "flickr_zip" ]; then
  torchrun \
    --nnodes=1 \
    --node_rank=0 \
    --master_addr=127.0.0.1 \
    --nproc_per_node=${GPUS} \
    --master_port=${MASTER_PORT} \
    eval/caption/evaluate_caption.py --checkpoint ${CHECKPOINT} --datasets flickr_zip "${ARGS[@]:2}"
fi

if  [ ${DATASET} == "flickr_backdoor_robust" ]; then
  torchrun \
    --nnodes=1 \
    --node_rank=0 \
    --master_addr=127.0.0.1 \
    --nproc_per_node=${GPUS} \
    --master_port=${MASTER_PORT} \
    eval/caption/evaluate_caption_robust.py --checkpoint ${CHECKPOINT} --datasets flickr "${ARGS[@]:2}"
fi

if  [ ${DATASET} == "flickr_backdoor" ]; then
  torchrun \
    --nnodes=1 \
    --node_rank=0 \
    --master_addr=127.0.0.1 \
    --nproc_per_node=${GPUS} \
    --master_port=${MASTER_PORT} \
    eval/caption/evaluate_caption.py --checkpoint ${CHECKPOINT} --datasets flickr_backdoor "${ARGS[@]:2}"
fi

if  [ ${DATASET} == "flickr_backdoor_zip" ]; then
  torchrun \
    --nnodes=1 \
    --node_rank=0 \
    --master_addr=127.0.0.1 \
    --nproc_per_node=${GPUS} \
    --master_port=${MASTER_PORT} \
    eval/caption/evaluate_caption.py --checkpoint ${CHECKPOINT} --datasets flickr_backdoor_zip "${ARGS[@]:2}"
fi
